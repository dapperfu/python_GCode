...
def square(t, A=1, f=1, D=0):
    ...
    D: non-zero center amplitude
    """
    square_ = A * scipy.signal.square(2 * np.pi * f * t) + D
    return square_
...
def sawtooth(t, A=1, f=1, D=0):
    ...
    D: non-zero center amplitude
    """
    sawtooth_ = A * scipy.signal.sawtooth(2 * np.pi * f * t, width=1) + D
    return sawtooth_
...
def triangle(t, A=1, f=1, D=0):
    ...
    D: non-zero center amplitude
    """
    triangle_ = A * scipy.signal.sawtooth(2 * np.pi * f * t, width=0.5) + D
    return triangle_
