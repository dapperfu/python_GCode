...
Sane default for the ```Triangle``` command. Designed so that ```draw_line()``` does something that is easly measureable.
...
default_points = np.array(
    [[0, 0], [a * np.sqrt(3), 0], [a * np.sqrt(3), a], [0, 0]]
)  # Start at origin.  # Draw long side along X axis.  # Draw the short side parallel to Y axis.  # Return to origin. Draw hypotenuse.


class Triangle(GCode):
    def __init__(
        self,
